"""
PrayerOffline - Complete Prayer Times Application
Beautiful glassmorphic design with proper architecture
"""
...
class StorageService:
    """Simple storage service for app settings"""
...
class DashboardView(ft.Column):
    ...
    def __init__(self, page: ft.Page, storage: StorageService):
        ...
        self.show_jamaat = storage.get_setting('show_jamaat', True)
        self.next_prayer = PrayerData.get_next_prayer()
    ...
    def _build_header(self) -> ft.Control:
        ...
        today = datetime.now()
        location = self.storage.get_setting('location', 'New York, USA')
    ...
    def cleanup(self):
        ...
        logger.debug("Dashboard view cleaned up")
...
class QiblaView(ft.Container):
    ...
    def __init__(self):
        ...
        logger.info("Qibla view initialized")
...
class SettingsView(ft.Container):
    ...
    def __init__(self, storage: StorageService, on_jamaat_toggle, on_theme_change):
        self.storage = storage
        self.on_jamaat_toggle = on_jamaat_toggle
        self.on_theme_change = on_theme_change
        
        current_theme = storage.get_setting('theme_mode', 'Light')
        location = storage.get_setting('location', 'New York, USA')
        calc_method = storage.get_setting('calculation_method', 'ISNA')
        notifications = storage.get_setting('notifications_enabled', True)
                        ...
                        controls=[
                            ...
                            ft.Row(
                                controls=[
                                    ...
                                    ft.Switch(
                                        value=storage.get_setting('show_jamaat', True),
                                        on_change=on_jamaat_toggle,
                            ...
                            ft.Column(
                                controls=[
                                    self._setting_row("📍 Location", location),
                                    self._setting_row("🧮 Method", calc_method),
                                    self._setting_row("🌙 Asr School", "Standard (Shafi)"),
                                    self._setting_row("🌍 Timezone", "America/New_York"),
                                    self._setting_row("🔔 Notifications", "Enabled" if notifications else "Disabled"),
                                ],
                                spacing=12
                            ),
...
class PrayerOfflineApp:
    """Main application with proper architecture"""
    
    def __init__(self, page: ft.Page, storage: StorageService):
        self.page = page
        self.storage = storage
        self.page.title = "PrayerOffline"
        self.page.padding = 0
        
        self.current_view = "home"
        self.current_dashboard = None
        
        logger.info("PrayerOfflineApp initialized")
    ...
    def initialize(self):
        ...
        try:
            ...
            logger.error(f"Error building app: {e}", exc_info=True)
            raise
    
    def build(self):
        """Build main app UI"""
        theme_mode = self.storage.get_setting('theme_mode', 'Light')
        is_dark = theme_mode == "Dark" or (theme_mode == "System" and self.page.theme_mode == ft.ThemeMode.DARK)
        ...
        # Content area
        self.current_dashboard = DashboardView(self.page, self.storage)
    ...
    def switch_view(self, view_name: str):
        ...
        try:
            ...
            self.page.update()
            logger.info(f"View switched to: {view_name}")
            
        except Exception as e:
            logger.error(f"Error switching view: {e}", exc_info=True)
            self._show_error("Failed to switch view")
    
    def _on_theme_change(self, e):
        """Handle theme change"""
        try:
            theme_mode = e.control.value
            self.storage.set_setting('theme_mode', theme_mode)
            ...
            self.page.show_snack_bar(
                ft.SnackBar(
                    content=ft.Text(f"Theme changed to {theme_mode}", color=ft.Colors.WHITE),
            ...
            self._show_error("Failed to change theme")
    
    def _on_jamaat_toggle(self, e):
        """Handle Jamaat toggle"""
        try:
            show_jamaat = e.control.value
            self.storage.set_setting('show_jamaat', show_jamaat)
            ...
            logger.info(f"Jamaat times visibility: {show_jamaat}")
            
        except Exception as e:
            logger.error(f"Error toggling jamaat: {e}", exc_info=True)
            self._show_error("Failed to toggle jamaat times")
        ...
        try:
            self.page.show_snack_bar(
                ft.SnackBar(
                    ...
                    bgcolor=ft.Colors.with_opacity(0.9, DesignTokens.ERROR)
                )
            )
            self.page.update()
        except Exception as e:
